...
A stripped-down sibling of the policy-prior CEM planner
(`policy_prior_CEM.py`). CEM there *iterates*: sample -> evaluate -> refit the
elite Gaussian, repeating until the distribution collapses and converges.
...
Unlike the CEM variant, we do NOT widen the prior: the sampling std is exactly
...
Like the CEM variant, the plan is NOT carried over / shifted between timesteps;
...
# reuse the shared rollout + policy prior from the CEM planner so the two
# variants stay in lock-step (same evaluation, same prior mapping).
from policy_prior_CEM import _rollout_returns, SACPrior
...
class RandomShootingPlanner:
    ...
    def _init_from_prior(self, state: np.ndarray):
        ...
        NOTE: sigma is the *raw* SAC std -- unlike the CEM variant we do not
